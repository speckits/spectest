...
export class InitCommand {
  ...
  async execute(targetPath: string): Promise<void> {
    ...
    const spectestDir = SPECTEST_DIR_NAME;
    const spectestPath = path.join(projectPath, spectestDir);
    ...
    if (!extendMode) {
      ...
      await this.createDirectoryStructure(spectestPath);
      await this.ensureTemplateFiles(spectestPath, config);
    }
  ...
  private async validate(
    projectPath: string,
    _spectestPath: string
  ): Promise<boolean> {
    const extendMode = await FileSystemUtils.directoryExists(_spectestPath);
  ...
  private async isToolConfigured(
    ...
    // A tool is only considered "configured by SpecTest" if its files contain SpecTest markers.
    ...
    // Helper to check if a file exists and contains SpecTest markers
    ...
    // Check if the tool has a config file with SpecTest markers
  ...
  private async generateFiles(
    ...
    await this.writeTemplateFiles(spectestPath, config, false);
  }
  ...
  private async ensureTemplateFiles(
    ...
    await this.writeTemplateFiles(spectestPath, config, true);
  }
  ...
  private async writeTemplateFiles(
    ...
    for (const template of templates) {
      const filePath = path.join(spectestPath, template.path);
  ...
  private async configureAITools(
    ...
    for (const toolId of toolIds) {
      ...
      if (slashConfigurator && slashConfigurator.isAvailable) {
        await slashConfigurator.generateAll(projectPath, spectestDir);
      }
    }
