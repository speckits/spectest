...
export class ArchiveCommand {
  async execute(
    ...
    const targetPath = '.';
    const changesDir = path.join(targetPath, 'spectest', 'changes');
    const archiveDir = path.join(changesDir, 'archive');
    const mainSpecsDir = path.join(targetPath, 'spectest', 'specs');

    // Check if changes directory exists
    try {
      await fs.access(changesDir);
    } catch {
      throw new Error("No SpecTest changes directory found. Run 'spectest init' first.");
    }
  ...
  private async writeUpdatedSpec(update: SpecUpdate, rebuilt: string, counts: { added: number; modified: number; removed: number; renamed: number }): Promise<void> {
    ...
    const specName = path.basename(path.dirname(update.target));
    console.log(`Applying changes to spectest/specs/${specName}/spec.md:`);
