...
export class ZshInstaller {
  ...
  async getInstallationPath(): Promise<{ path: string; isOhMyZsh: boolean }> {
    ...
    if (isOhMyZsh) {
      ...
      return {
        path: path.join(this.homeDir, '.oh-my-zsh', 'custom', 'completions', '_spectest'),
        isOhMyZsh: true,
      };
    } else {
      // Standard Zsh completions directory
      return {
        path: path.join(this.homeDir, '.zsh', 'completions', '_spectest'),
        isOhMyZsh: false,
      };
    }
  }
  ...
  async configureZshrc(completionsDir: string): Promise<boolean> {
    ...
    if (process.env.SPECTEST_NO_AUTO_CONFIG === '1') {
      return false;
    }
  ...
  /**
   * Check if .zshrc has SpecTest configuration markers
   *
  ...
  async uninstall(): Promise<{ success: boolean; message: string }> {
    try {
      ...
      if (zshrcCleaned && !isOhMyZsh) {
        messages.push('Removed SpecTest configuration from ~/.zshrc');
      }
