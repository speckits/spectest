...
/**
 * Generates Zsh completion scripts for the SpecTest CLI.
 * Follows Zsh completion system conventions using the _spectest function.
 */
export class ZshGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    // Header comment
    script.push('#compdef spectest');
    script.push('');
    script.push('# Zsh completion script for SpecTest CLI');
    script.push('# Auto-generated - do not edit manually');
    script.push('');

    // Main completion function
    script.push('_spectest() {');
    ...
    script.push('    command)');
    script.push('      _describe "spectest command" commands');
    script.push('      ;;');
    script.push('    args)');
    ...
    for (const cmd of commands) {
      ...
      script.push(`          _spectest_${this.sanitizeFunctionName(cmd.name)}`);
      script.push('          ;;');
    }
    ...
    script.push('compdef _spectest spectest');
    script.push('');
  ...
  private generateDynamicCompletionHelpers(): string[] {
    ...
    lines.push('# Use spectest __complete to get available changes');
    lines.push('_spectest_complete_changes() {');
    lines.push('  local -a changes');
    lines.push('  while IFS=$\'\\t\' read -r id desc; do');
    lines.push('    changes+=("$id:$desc")');
    lines.push('  done < <(spectest __complete changes 2>/dev/null)');
    lines.push('  _describe "change" changes');
    lines.push('}');
    lines.push('');

    // Helper function for completing spec IDs
    lines.push('# Use spectest __complete to get available specs');
    lines.push('_spectest_complete_specs() {');
    lines.push('  local -a specs');
    lines.push('  while IFS=$\'\\t\' read -r id desc; do');
    lines.push('    specs+=("$id:$desc")');
    lines.push('  done < <(spectest __complete specs 2>/dev/null)');
    ...
    lines.push('_spectest_complete_items() {');
  ...
  private generateCommandFunction(cmd: CommandDefinition): string[] {
    const funcName = `_spectest_${this.sanitizeFunctionName(cmd.name)}`;
    const lines: string[] = [];
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        ...
        lines.push(`          _spectest_${this.sanitizeFunctionName(cmd.name)}_${this.sanitizeFunctionName(subcmd.name)}`);
        lines.push('          ;;');
      }
  ...
  private generateSubcommandFunction(parentName: string, subcmd: CommandDefinition): string[] {
    const funcName = `_spectest_${this.sanitizeFunctionName(parentName)}_${this.sanitizeFunctionName(subcmd.name)}`;
    const lines: string[] = [];
  ...
  private generatePositionalSpec(positionalType?: string): string {
    switch (positionalType) {
      case 'change-id':
        return "'*: :_spectest_complete_changes'";
      case 'spec-id':
        return "'*: :_spectest_complete_specs'";
      case 'change-or-spec-id':
        return "'*: :_spectest_complete_items'";
      case 'path':
        return "'*:path:_files'";
      case 'shell':
        return "'*:shell:(zsh)'";
