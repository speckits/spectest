...
export class ViewCommand {
  async execute(targetPath: string = '.'): Promise<void> {
    const spectestDir = path.join(targetPath, 'spectest');
    
    if (!fs.existsSync(spectestDir)) {
      console.error(chalk.red('No spectest directory found'));
      process.exit(1);
    }
    ...
    const changesData = await this.getChangesData(spectestDir);
    const specsData = await this.getSpecsData(spectestDir);
    ...
    console.log('\n' + '═'.repeat(60));
    console.log(chalk.dim(`\nUse ${chalk.white('spectest list --changes')} or ${chalk.white('spectest list --specs')} for detailed views`));
  }

  private async getChangesData(spectestDir: string): Promise<{
    active: Array<{ name: string; progress: { total: number; completed: number } }>;
    completed: Array<{ name: string }>;
  }> {
    const changesDir = path.join(spectestDir, 'changes');
  ...
  private async getSpecsData(spectestDir: string): Promise<Array<{ name: string; requirementCount: number }>> {
    const specsDir = path.join(spectestDir, 'specs');
