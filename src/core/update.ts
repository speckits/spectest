...
import { SPECTEST_DIR_NAME } from './config.js';
...
export class UpdateCommand {
  async execute(projectPath: string): Promise<void> {
    ...
    const spectestDirName = SPECTEST_DIR_NAME;
    const spectestPath = path.join(resolvedProjectPath, spectestDirName);

    // 1. Check spectest directory exists
    if (!await FileSystemUtils.directoryExists(spectestPath)) {
      throw new Error(`No SpecTest directory found. Run 'spectest init' first.`);
    }

    // 2. Update AGENTS.md (full replacement)
    const agentsPath = path.join(spectestPath, 'AGENTS.md');
    ...
    for (const configurator of configurators) {
      ...
      try {
        ...
        await configurator.configure(resolvedProjectPath, spectestPath);
    ...
    const instructionFiles: string[] = ['spectest/AGENTS.md'];
    ...
    summaryParts.push(
      `Updated SpecTest instructions (${instructionFiles.join(', ')})`
    );
