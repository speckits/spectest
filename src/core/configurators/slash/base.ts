...
export abstract class SlashCommandConfigurator {
  ...
  protected async updateBody(filePath: string, body: string): Promise<void> {
    ...
    const endIndex = content.indexOf(SPECTEST_MARKERS.end);

    if (startIndex === -1 || endIndex === -1 || endIndex <= startIndex) {
      throw new Error(`Missing SpecTest markers in ${filePath}`);
    }

    const before = content.slice(0, startIndex + SPECTEST_MARKERS.start.length);
