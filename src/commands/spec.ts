...
const SPECS_DIR = 'spectest/specs';
...
function filterSpec(spec: Spec, options: ShowOptions): Spec {
  ...
  const metadata = spec.metadata ?? { version: '1.0.0', format: 'spectest' as const };
...
export class SpecCommand {
  ...
  async show(specId?: string, options: ShowOptions = {}): Promise<void> {
    ...
    if (!existsSync(specPath)) {
      throw new Error(`Spec '${specId}' not found at spectest/specs/${specId}/spec.md`);
    }
    ...
    if (options.json) {
      ...
      const output = {
        ...
        metadata: parsed.metadata ?? { version: '1.0.0', format: 'spectest' as const },
      };
...
export function registerSpecCommand(rootProgram: typeof program) {
  const specCommand = rootProgram
    .command('spec')
    .description('Manage and view test specifications');

  // Deprecation notice for noun-based commands
  specCommand.hook('preAction', () => {
    console.error('Warning: The "spectest spec ..." commands are deprecated. Prefer verb-first commands (e.g., "spectest show", "spectest validate --specs").');
  });
