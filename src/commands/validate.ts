...
export class ValidateCommand {
  ...
  private printNonInteractiveHint(): void {
    ...
    console.error('  spectest validate --all');
    console.error('  spectest validate --changes');
    console.error('  spectest validate --specs');
    console.error('  spectest validate <item-name>');
  ...
  private async validateDirectItem(itemName: string, opts: { typeOverride?: ItemType; strict: boolean; json: boolean }): Promise<void> {
    ...
    if (!opts.typeOverride && isChange && isSpec) {
      ...
      console.error('Pass --type change|spec, or use: spectest change validate / spectest spec validate');
      process.exitCode = 1;
      return;
    }
  ...
  private async validateByType(type: ItemType, id: string, opts: { strict: boolean; json: boolean }): Promise<void> {
    ...
    if (type === 'change') {
      const changeDir = path.join(process.cwd(), 'spectest', 'changes', id);
    ...
    const file = path.join(process.cwd(), 'spectest', 'specs', id, 'spec.md');
    const start = Date.now();
  ...
  private printNextSteps(type: ItemType): void {
    ...
    if (type === 'change') {
      ...
      bullets.push('- Debug parsed deltas: spectest change show <id> --json --deltas-only');
    } else {
  ...
  private async runBulkValidation(scope: { changes: boolean; specs: boolean }, opts: { strict: boolean; json: boolean; concurrency?: string }): Promise<void> {
    ...
    const concurrency = normalizeConcurrency(opts.concurrency) ?? normalizeConcurrency(process.env.SPECTEST_CONCURRENCY) ?? DEFAULT_CONCURRENCY;
    const validator = new Validator(opts.strict);
