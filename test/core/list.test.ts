...
describe('ListCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `spectest-list-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should handle missing spectest/changes directory', async () => {
      const listCommand = new ListCommand();
      
      await expect(listCommand.execute(tempDir, 'changes')).rejects.toThrow(
        "No SpecTest changes directory found. Run 'spectest init' first."
      );
    });

    it('should handle empty changes directory', async () => {
      const changesDir = path.join(tempDir, 'spectest', 'changes');
