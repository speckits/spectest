...
describe('UpdateCommand', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `spectest-test-${randomUUID()}`);
    await fs.mkdir(testDir, { recursive: true });

    // Create spectest directory
    const spectestDir = path.join(testDir, 'spectest');
    await fs.mkdir(spectestDir, { recursive: true });
...
More content after.`;
    ...
    expect(logMessage).toContain(
      'Updated SpecTest instructions (spectest/AGENTS.md'
    );
...
<!-- SPECTEST:END -->`;
    ...
    expect(updated).toContain('name: SpecTest: Proposal');
    expect(updated).toContain('**Guardrails**');
    expect(updated).toContain(
      'Validate with `spectest validate <id> --strict`'
    );
    ...
    expect(logMessage).toContain('AGENTS.md (created)');
    expect(logMessage).toContain(
      'Updated slash commands: .claude/commands/spectest/proposal.md'
    );
...
`;
    ...
    expect(logMessage).toContain('AGENTS.md (created)');
    expect(logMessage).toContain(
      'Updated slash commands: .qwen/commands/spectest-apply.toml'
    );
...
<!-- SPECTEST:END -->`;
    ...
    const updated = await fs.readFile(proposalPath, 'utf-8');
    expect(updated).toContain('# SpecTest: Proposal');
    expect(updated).toContain('**Guardrails**');
    expect(updated).toContain(
      'Validate with `spectest validate <id> --strict`'
    );
    ...
    expect(logMessage).toContain('AGENTS.md (created)');
    expect(logMessage).toContain(
      'Updated slash commands: .clinerules/workflows/spectest-proposal.md'
    );
  ...
  it('should refresh existing Cursor slash command files', async () => {
    const cursorPath = path.join(testDir, '.cursor/commands/spectest-apply.md');
...
<!-- SPECTEST:END -->`;
    ...
    expect(logMessage).toContain('AGENTS.md (created)');
    expect(logMessage).toContain(
      'Updated slash commands: .cursor/commands/spectest-apply.md'
    );
...
<!-- SPECTEST:END -->`;
    ...
    expect(logMessage).toContain('AGENTS.md (created)');
    expect(logMessage).toContain(
      'Updated slash commands: .opencode/command/spectest-apply.md'
    );
...
<!-- SPECTEST:END -->`;
    ...
    const [logMessage] = consoleSpy.mock.calls[0];
    expect(logMessage).toContain(
      'Updated slash commands: .kilocode/workflows/spectest-apply.md'
    );
...
<!-- SPECTEST:END -->`;
    ...
    const [logMessage] = consoleSpy.mock.calls[0];
    expect(logMessage).toContain(
      'Updated slash commands: .windsurf/workflows/spectest-apply.md'
    );
    consoleSpy.mockRestore();
  });
...
<!-- SPECTEST:END -->`;
    ...
    const [logMessage] = consoleSpy.mock.calls[0];
    expect(logMessage).toContain(
      'Updated slash commands: .agent/workflows/spectest-apply.md'
    );
    consoleSpy.mockRestore();
  });
  ...
  it('should refresh existing Codex prompts', async () => {
    ...
    const [logMessage] = consoleSpy.mock.calls[0];
    expect(logMessage).toContain(
      'Updated slash commands: .codex/prompts/spectest-apply.md'
    );
...
<!-- SPECTEST:END -->`;
    ...
    const [logMessage] = consoleSpy.mock.calls[0];
    expect(logMessage).toContain(
      'Updated slash commands: .github/prompts/spectest-apply.prompt.md'
    );
...
`;
    ...
    const [logMessage] = consoleSpy.mock.calls[0];
    expect(logMessage).toContain(
      'Updated slash commands: .gemini/commands/spectest/proposal.toml'
    );
...
`;
    ...
    const [logMessage] = consoleSpy.mock.calls[0];
    expect(logMessage).toContain(
      'Updated slash commands: .iflow/commands/spectest-proposal.md'
    );
...
<!-- SPECTEST:END -->`;
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('.factory/commands/spectest-proposal.md')
    );
...
<!-- SPECTEST:END -->`;
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('.amazonq/prompts/spectest-apply.md')
    );
...
<!-- SPECTEST:END -->`;
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining('.augment/commands/spectest-apply.md')
    );
...
<!-- SPECTEST:END -->`;
    ...
    expect(logMessage).toContain('AGENTS.md (created)');
    expect(logMessage).toContain(
      'Updated slash commands: .codebuddy/commands/spectest/proposal.md'
    );
...
<!-- SPECTEST:END -->`;
    ...
    expect(logMessage).toContain('AGENTS.md (created)');
    expect(logMessage).toContain(
      'Updated slash commands: .crush/commands/spectest/proposal.md'
    );
...
<!-- SPECTEST:END -->`;
    ...
    // For slash commands, only the content between SpecTest markers is updated
    ...
    expect(logMessage).toContain('AGENTS.md (created)');
    expect(logMessage).toContain(
      'Updated slash commands: .cospec/spectest/commands/spectest-proposal.md'
    );
...
<!-- SPECTEST:END -->`;
    ...
    expect(logMessage).toContain('AGENTS.md (created)');
    expect(logMessage).toContain(
      'Updated slash commands: .qoder/commands/spectest/proposal.md'
    );
...
<!-- SPECTEST:END -->`;
    ...
    expect(logMessage).toContain('AGENTS.md (created)');
    expect(logMessage).toContain(
      'Updated slash commands: .roo/commands/spectest-proposal.md'
    );
...
More instructions after.`;
  ...
  it('should preserve CoStrict content outside markers during update', async () => {
    ...
    expect(updated).not.toContain('Old body');
    expect(updated).toContain('Validate with `spectest validate <id> --strict`');
  });
  ...
  it('should handle no AI tool files present', async () => {
    ...
    // Should only update SpecTest instructions
    const [logMessage] = consoleSpy.mock.calls[0];
    expect(logMessage).toContain(
      'Updated SpecTest instructions (spectest/AGENTS.md'
    );
...
<!-- SPECTEST:END -->`
    ...
    const applyExists = await FileSystemUtils.fileExists(
      path.join(testDir, '.claude/commands/spectest/apply.md')
    );
    const archiveExists = await FileSystemUtils.fileExists(
      path.join(testDir, '.claude/commands/spectest/archive.md')
    );
  ...
  it('should update AGENTS.md in spectest directory', async () => {
    // Execute update command
    await updateCommand.execute(testDir);
    ...
    expect(content).toContain('# SpecTest Instructions');
  });
  ...
  it('should refresh root AGENTS.md while preserving surrounding content', async () => {
    ...
    expect(updated).toContain("@/spectest/AGENTS.md");
    expect(updated).toContain('spectest update');
    expect(updated).not.toContain('Old content');

    const [logMessage] = consoleSpy.mock.calls[0];
    expect(logMessage).toContain(
      'Updated SpecTest instructions (spectest/AGENTS.md, AGENTS.md)'
    );
  ...
  it('should throw error if spectest directory does not exist', async () => {
    // Remove spectest directory
    await fs.rm(path.join(testDir, 'spectest'), {
      recursive: true,
      force: true,
    });
    ...
    await expect(updateCommand.execute(testDir)).rejects.toThrow(
      "No SpecTest directory found. Run 'spectest init' first."
    );
  });
