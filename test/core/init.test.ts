...
describe('InitCommand', () => {
  ...
  beforeEach(async () => {
    testDir = path.join(os.tmpdir(), `spectest-init-test-${Date.now()}`);
  ...
  describe('execute', () => {
    it('should create SpecTest directory structure', async () => {
      queueSelections('claude', DONE);

      await initCommand.execute(testDir);

      const spectestPath = path.join(testDir, 'spectest');
      expect(await directoryExists(spectestPath)).toBe(true);
      expect(await directoryExists(path.join(spectestPath, 'specs'))).toBe(
        true
      );
      expect(await directoryExists(path.join(spectestPath, 'changes'))).toBe(
        true
      );
      expect(
        await directoryExists(path.join(spectestPath, 'changes', 'archive'))
      ).toBe(true);
    });
    ...
    it('should create AGENTS.md and project.md', async () => {
      ...
      const spectestPath = path.join(testDir, 'spectest');
      expect(await fileExists(path.join(spectestPath, 'AGENTS.md'))).toBe(true);
      expect(await fileExists(path.join(spectestPath, 'project.md'))).toBe(
        true
      );

      const agentsContent = await fs.readFile(
        path.join(spectestPath, 'AGENTS.md'),
        'utf-8'
      );
      expect(agentsContent).toContain('SpecTest Instructions');

      const projectContent = await fs.readFile(
        path.join(spectestPath, 'project.md'),
        'utf-8'
      );
    ...
    it('should create Windsurf workflows when Windsurf is selected', async () => {
      ...
      const wsApply = path.join(
        testDir,
        '.windsurf/workflows/spectest-apply.md'
      );
      const wsArchive = path.join(
        testDir,
        '.windsurf/workflows/spectest-archive.md'
      );
      ...
      expect(proposalContent).toContain('description: Scaffold a new test change and validate strictly.');
      ...
      expect(applyContent).toContain('description: Implement an approved test change and keep tasks in sync.');
      expect(applyContent).toContain('auto_execution_mode: 3');
      expect(applyContent).toContain('<!-- SPECTEST:START -->');
      expect(applyContent).toContain('Track these steps as TODOs and complete them one by one.');

      const archiveContent = await fs.readFile(wsArchive, 'utf-8');
      expect(archiveContent).toContain('---');
      expect(archiveContent).toContain('description: Archive a deployed test change and update test specs.');
    ...
    it('should create Antigravity workflows when Antigravity is selected', async () => {
      ...
      const agProposal = path.join(
        testDir,
        '.agent/workflows/spectest-proposal.md'
      );
      const agApply = path.join(
        testDir,
        '.agent/workflows/spectest-apply.md'
      );
      const agArchive = path.join(
        testDir,
        '.agent/workflows/spectest-archive.md'
      );
    ...
    it('should create Claude slash command files with templates', async () => {
      ...
      const claudeProposal = path.join(
        testDir,
        '.claude/commands/spectest/proposal.md'
      );
      const claudeApply = path.join(
        testDir,
        '.claude/commands/spectest/apply.md'
      );
      const claudeArchive = path.join(
        testDir,
        '.claude/commands/spectest/archive.md'
      );
      ...
      expect(applyContent).toContain('name: SpecTest: Apply');
      expect(applyContent).toContain('Track these steps as TODOs and complete them one by one.');

      const archiveContent = await fs.readFile(claudeArchive, 'utf-8');
      expect(archiveContent).toContain('name: SpecTest: Archive');
      expect(archiveContent).toContain('spectest archive <id>');
      expect(archiveContent).toContain(
    ...
    it('should create Cursor slash command files with templates', async () => {
      ...
      const cursorProposal = path.join(
        testDir,
        '.cursor/commands/spectest-proposal.md'
      );
      const cursorApply = path.join(
        testDir,
        '.cursor/commands/spectest-apply.md'
      );
      const cursorArchive = path.join(
        testDir,
        '.cursor/commands/spectest-archive.md'
      );
      ...
      expect(archiveContent).toContain('name: /spectest-archive');
      expect(archiveContent).toContain('spectest list --specs');
    });
    ...
    it('should create Gemini CLI TOML files when selected', async () => {
      ...
      const geminiApply = path.join(
        testDir,
        '.gemini/commands/spectest/apply.toml'
      );
      const geminiArchive = path.join(
        testDir,
        '.gemini/commands/spectest/archive.toml'
      );
    ...
    it('should create IFlow CLI slash command files with templates', async () => {
      ...
      const iflowApply = path.join(
        testDir,
        '.iflow/commands/spectest-apply.md'
      );
      const iflowArchive = path.join(
        testDir,
        '.iflow/commands/spectest-archive.md'
      );
    ...
    it('should create OpenCode slash command files with templates', async () => {
      ...
      const openCodeProposal = path.join(
        testDir,
        '.opencode/command/spectest-proposal.md'
      );
      const openCodeApply = path.join(
        testDir,
        '.opencode/command/spectest-apply.md'
      );
      const openCodeArchive = path.join(
        testDir,
        '.opencode/command/spectest-archive.md'
      );
    ...
    it('should create Qwen configuration and slash command files with templates', async () => {
      ...
      const proposalPath = path.join(
        testDir,
        '.qwen/commands/spectest-proposal.toml'
      );
      ...
      const archivePath = path.join(
        testDir,
        '.qwen/commands/spectest-archive.toml'
      );
    ...
    it('should create Cline workflow files with templates', async () => {
      ...
      const clineProposal = path.join(
        testDir,
        '.clinerules/workflows/spectest-proposal.md'
      );
      const clineApply = path.join(
        testDir,
        '.clinerules/workflows/spectest-apply.md'
      );
      const clineArchive = path.join(
        testDir,
        '.clinerules/workflows/spectest-archive.md'
      );
      ...
      expect(proposalContent).toContain('Scaffold a new test change and validate strictly.');
      expect(proposalContent).toContain('<!-- SPECTEST:START -->');
      expect(proposalContent).toContain('**Guardrails**');

      const applyContent = await fs.readFile(clineApply, 'utf-8');
      expect(applyContent).toContain('# SpecTest: Apply');
      expect(applyContent).toContain('Implement an approved test change and keep tasks in sync.');
      expect(applyContent).toContain('Track these steps as TODOs and complete them one by one.');

      const archiveContent = await fs.readFile(clineArchive, 'utf-8');
      expect(archiveContent).toContain('# SpecTest: Archive');
      expect(archiveContent).toContain('Archive a deployed test change and update test specs.');
      expect(archiveContent).toContain('spectest archive <id>');
    });
    ...
    it('should create Factory slash command files with templates', async () => {
      ...
      const factoryProposal = path.join(
        testDir,
        '.factory/commands/spectest-proposal.md'
      );
      const factoryApply = path.join(
        testDir,
        '.factory/commands/spectest-apply.md'
      );
      const factoryArchive = path.join(
        testDir,
        '.factory/commands/spectest-archive.md'
      );
      ...
      expect(proposalContent).toContain('description: Scaffold a new test change and validate strictly.');
      expect(proposalContent).toContain('argument-hint: test change request or description');
      expect(proposalContent).toContain('<!-- SPECTEST:START -->');
      expect(
    ...
    it('should create Codex prompts with templates and placeholders', async () => {
      ...
      const proposalPath = path.join(
        testDir,
        '.codex/prompts/spectest-proposal.md'
      );
      const applyPath = path.join(
        testDir,
      ...
      const archivePath = path.join(
        testDir,
        '.codex/prompts/spectest-archive.md'
      );
    ...
    it('should create Kilo Code workflows with templates', async () => {
      ...
      const proposalPath = path.join(
        testDir,
        '.kilocode/workflows/spectest-proposal.md'
      );
      const applyPath = path.join(
        testDir,
      ...
      const archivePath = path.join(
        testDir,
        '.kilocode/workflows/spectest-archive.md'
      );
    ...
    it('should create GitHub Copilot prompt files with templates', async () => {
      ...
      const proposalPath = path.join(
        testDir,
        '.github/prompts/spectest-proposal.prompt.md'
      );
      const applyPath = path.join(
        testDir,
        '.github/prompts/spectest-apply.prompt.md'
      );
      const archivePath = path.join(
        testDir,
        '.github/prompts/spectest-archive.prompt.md'
      );
    ...
    it('should add new tool when SpecTest already exists', async () => {
    ...
    it('should recreate deleted spectest/AGENTS.md in extend mode', async () => {
    ...
    it('should recreate deleted spectest/project.md in extend mode', async () => {
      await testFileRecreationInExtendMode(
        testDir,
        initCommand,
        'spectest/project.md',
    ...
    it('should preserve existing template files in extend mode', async () => {
      ...
      const agentsPath = path.join(testDir, 'spectest', 'AGENTS.md');
      ...
      expect(content).not.toContain('SpecTest Instructions');
    });
    ...
    it('should handle non-existent target directory', async () => {
      ...
      const spectestPath = path.join(newDir, 'spectest');
      expect(await directoryExists(spectestPath)).toBe(true);
    });
  ...
  describe('AI tool selection', () => {
    ...
    it('should create Amazon Q Developer prompt files with templates', async () => {
      ...
      const proposalPath = path.join(
        testDir,
        '.amazonq/prompts/spectest-proposal.md'
      );
      const applyPath = path.join(
        testDir,
        '.amazonq/prompts/spectest-apply.md'
      );
      const archivePath = path.join(
        testDir,
        '.amazonq/prompts/spectest-archive.md'
      );
    ...
    it('should create Auggie slash command files with templates', async () => {
      ...
      const auggieProposal = path.join(
        testDir,
        '.augment/commands/spectest-proposal.md'
      );
      const auggieApply = path.join(
        testDir,
        '.augment/commands/spectest-apply.md'
      );
      const auggieArchive = path.join(
        testDir,
        '.augment/commands/spectest-archive.md'
      );
    ...
    it('should create CodeBuddy slash command files with templates', async () => {
      ...
      const codeBuddyProposal = path.join(
        testDir,
        '.codebuddy/commands/spectest/proposal.md'
      );
      const codeBuddyApply = path.join(
        testDir,
        '.codebuddy/commands/spectest/apply.md'
      );
      const codeBuddyArchive = path.join(
        testDir,
        '.codebuddy/commands/spectest/archive.md'
      );
    ...
    it('should create Crush slash command files with templates', async () => {
      ...
      const crushProposal = path.join(
        testDir,
        '.crush/commands/spectest/proposal.md'
      );
      const crushApply = path.join(
        testDir,
        '.crush/commands/spectest/apply.md'
      );
      const crushArchive = path.join(
        testDir,
        '.crush/commands/spectest/archive.md'
      );
      ...
      expect(proposalContent).toContain('description: Scaffold a new test change and validate strictly.');
      expect(proposalContent).toContain('category: SpecTest');
      expect(proposalContent).toContain('tags: [spectest, test-change]');
      expect(proposalContent).toContain('<!-- SPECTEST:START -->');
      expect(proposalContent).toContain('**Guardrails**');
      ...
      expect(applyContent).toContain('name: SpecTest: Apply');
      expect(applyContent).toContain('description: Implement an approved test change and keep tasks in sync.');
      expect(applyContent).toContain('category: SpecTest');
      expect(applyContent).toContain('tags: [spectest, apply]');
      expect(applyContent).toContain('Track these steps as TODOs and complete them one by one.');
      ...
      expect(archiveContent).toContain('name: SpecTest: Archive');
      expect(archiveContent).toContain('description: Archive a deployed test change and update test specs.');
      expect(archiveContent).toContain('category: SpecTest');
      expect(archiveContent).toContain('tags: [spectest, archive]');
      expect(archiveContent).toContain('spectest archive <id> --yes');
    });
    ...
    it('should create CoStrict slash command files with templates', async () => {
      ...
      const costrictApply = path.join(
        testDir,
        '.cospec/spectest/commands/spectest-apply.md'
      );
      const costrictArchive = path.join(
        testDir,
        '.cospec/spectest/commands/spectest-archive.md'
      );
    ...
    it('should create RooCode slash command files with templates', async () => {
      ...
      const rooProposal = path.join(
        testDir,
        '.roo/commands/spectest-proposal.md'
      );
      const rooApply = path.join(
        testDir,
        '.roo/commands/spectest-apply.md'
      );
      const rooArchive = path.join(
        testDir,
        '.roo/commands/spectest-archive.md'
      );
    ...
    it('should create Qoder slash command files with templates', async () => {
      ...
      const qoderProposal = path.join(
        testDir,
        '.qoder/commands/spectest/proposal.md'
      );
      const qoderApply = path.join(
        testDir,
        '.qoder/commands/spectest/apply.md'
      );
      const qoderArchive = path.join(
        testDir,
        '.qoder/commands/spectest/archive.md'
      );
  ...
  describe('already configured detection', () => {
    it('should NOT show tools as already configured in fresh project with existing CLAUDE.md', async () => {
      // Simulate user having their own CLAUDE.md before running spectest init
      const claudePath = path.join(testDir, 'CLAUDE.md');
    ...
    it('should NOT show already configured for Codex in fresh init even with global prompts', async () => {
      ...
      await fs.writeFile(
        path.join(codexPromptsDir, 'spectest-proposal.md'),
        '# Existing prompt\n'
      );
      ...
      vi.spyOn(fs, 'writeFile').mockImplementation(
        async (filePath: any, ...args: any[]) => {
          if (
            ...
            filePath.includes('.spectest-test-')
          ) {
