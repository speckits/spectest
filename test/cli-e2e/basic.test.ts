...
async function prepareFixture(fixtureName: string): Promise<string> {
  const base = await fs.mkdtemp(path.join(tmpdir(), 'spectest-cli-e2e-'));
  tempRoots.push(base);
...
describe('spectest CLI e2e basics', () => {
  it('shows help output', async () => {
    const result = await runCLI(['--help']);
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('Usage: spectest');
  ...
  describe('init command non-interactive options', () => {
    it('initializes with --tools all option', async () => {
      ...
      const cursorProposal = path.join(emptyProjectDir, '.cursor/commands/spectest-proposal.md');
